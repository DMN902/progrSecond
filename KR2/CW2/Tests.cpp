...
bool StartTests() {
	...
	bool flagAdd = false;
	bool flagDel = false;
	CreateTestVector(resultVector, ControlSize);			//Создание вектора с исходными значениями
	CreateTestVector(testVector, TestSize);					//Создание вектора с результативными значениями 
	Add(testVector, true);
